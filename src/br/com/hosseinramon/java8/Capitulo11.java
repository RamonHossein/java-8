...
public class Capitulo11 {
	...
	public static void main(String ... args) {
		...
		// Soma de todos os produtos de payment1 - versão 1
		payment1.getProducts().stream()
				.map(Product::getPrice)
				.reduce(BigDecimal::add)
				.ifPresent(System.out::println);
		
		// Soma de todos os produtos de payment1 - versão 2
		BigDecimal total1 = payment1.getProducts().stream()
				.map(Product::getPrice)
				.reduce(BigDecimal.ZERO, BigDecimal::add);
		
		// Soma de todos os produtos de todos os pagamentos - versão 1
		Stream<BigDecimal> priceStream = payments.stream()
				.map(p -> p.getProducts().stream()
						.map(Product::getPrice)
						.reduce(BigDecimal.ZERO, BigDecimal::add));
		
		// Soma de todos os produtos de todos os pagamentos - versão 2
		...
		// Cria um Stream<BigDecimal> com todos os valores de produtos - versão 1
		Stream<BigDecimal> priceOfEachproduct = payments.stream()
				.flatMap(p -> p.getProducts().stream()
						.map(Product::getPrice));
		
		// Cria um Stream<BigDecimal> com todos os valores de produtos - versão 2
		Function<Payment, Stream<BigDecimal>> mapper = p -> p.getProducts().stream()
				.map(Product::getPrice);
		
		// Soma de todos os produtos de todos os pagamentos - versão 3
		...
		// Calcula o número de meses pagos - versão 1
		long meses1 = ChronoUnit.MONTHS.between(s1.getBegin(), LocalDateTime.now());
		
		// Calcula o número de meses pagos - versão 2
