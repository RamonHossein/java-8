...
public class Capitulo08 {
	...
	public static void main(String ... args) throws IOException {
		...
		Usuario user3 = new Usuario("Guilherme Silveira", 190);
 		...
 		// Filtra os usuários com mais de 100 pontos e ordená-los
 		List<Usuario> filtradosOrdenados = usuarios.stream()
 				.filter(u -> u.getPontos() > 100)
 				.sorted(Comparator.comparing(Usuario::getNome))
 				.collect(Collectors.toList());
 		
 		// Retorna aleatóriamente qualquer usuário com mais de 100 pontos
 		Optional<Usuario> usuarioOptional = usuarios.stream()
 				.filter(u -> u.getPontos() > 100)
 				.findAny();
 		
 		// O stream executará uma tarefa sempre que processar um elemento (com findAny)
 		usuarios.stream().filter(u -> u.getPontos() > 100)
 				.peek(System.out::println)
 				.findAny();
 		
 		// O stream executará uma tarefa sempre que processar um elemento (sem findAny)
 		usuarios.stream().filter(u -> u.getPontos() > 100)
 				.peek(System.out::println);
 		
 		// O stream executará uma tarefa sempre que processar um elemento (com sorted)
 		...
 		// Retorna o usuário com a pontuação maxíma
 		Usuario pontuacaoMaxima = usuarios.stream()
 				.max(Comparator.comparing(Usuario::getPontos))
 				.get();
 		
 		// Retorna a soma da pontuação de todos os usuários - versão 1
 		int pontuacaoTotal1 = usuarios.stream()
 				.mapToInt(Usuario::getPontos)
 				.sum();
 		
 		// Retorna a soma da pontuação de todos os usuários - versão 2
 		int pontuacaoTotal2 = usuarios.stream()
 				.mapToInt(Usuario::getPontos)
 				.reduce(0, (a, b) -> a + b);
 		
 		// Retorna a soma da pontuação de todos os usuários - versão 3
 		int pontuacaoTotal3 = usuarios.stream()
 				.mapToInt(Usuario::getPontos)
 				.reduce(0, Integer::sum);
 		
 		// Retorna a soma da pontuação de todos os usuários - versão 4
 		int pontuacaoTotal4 = usuarios.stream()
 				.reduce(0, (atual, u) -> atual + u.getPontos(), Integer::sum);
 		
 		// Retorna a multiplicação da pontuação de todos os usuários
 		int multiplicacao = usuarios.stream()
 				.mapToInt(Usuario::getPontos)
 				.reduce(1, (a, b) -> a * b);
 		
 		// Percorrendo usuários com um iterator
 		usuarios.stream().iterator()
 				.forEachRemaining(System.out::println);
 		
 		// Verifica se algum usuário é moderador
 		boolean hasModerador = usuarios.stream()
 				.anyMatch(Usuario::isModerador);
 		
 		// Limita a criação de números randomicos a 100
 		...
 		// Imprimi os 10 primeiros números Fibonacci
 		...
 		// Imprimi os 10 primeiros números
 		...
 		// Lista todos os arquivos '.java' de um diretório
 		Files.list(Paths.get("./src/br/com/hosseinramon/java8"))
 				.filter(p -> p.toString().endsWith(".java"))
 				.forEach(System.out::println);
 		
 		// Mostra todo os conteúdo dos '.java' de um diretório - versão 1
 		Files.list(Paths.get("./src/br/com/hosseinramon/java8"))
			.filter(p -> p.toString().endsWith(".java"))
			.map(p -> lines(p))
			.forEach(System.out::println);
	
 		// Imprimi todo os conteúdo dos '.java' de um diretório - versão 2
 		Stream<Stream<String>> strings1 = 
 				Files.list(Paths.get("./src/br/com/hosseinramon/java8"))
 						.filter(p -> p.toString().endsWith(".java"))
 						.map(p -> lines(p));
 		
 		// Imprimi todo os conteúdo dos '.java' de um diretório - versão 3
 		...
 		// Imprimi todos os usuários de ambos os grupos sem nenhuma repetição
 		groups.stream().flatMap(g -> g.getUsuario().stream())
 				.distinct()
 				.forEach(System.out::println);
	}
	
	// Classe anônima utilizada para validar o 'lines'
